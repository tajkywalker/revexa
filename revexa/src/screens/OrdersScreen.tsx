...
export default function OrdersScreen({ onSelectOrder }: Props) {
  ...
  const loadOrders = useCallback(() => {
    let list: Order[] = filter === 'dnes' ? getOrders().filter(o => o.scheduledDate === todayISO()) : getOrders(filter || undefined);
    if (search.trim()) {
  ...
  function createOrder() {
    ...
    const order: Order = { id:uid(), customerId:newOrder.customerId, chimneyId:'', orderNumber:`#${String(count).padStart(4,'0')}`, status:'nova', scheduledDate:newOrder.scheduledDate, completedAt:'', address:newOrder.address || `${customer?.street ?? ''}, ${customer?.city ?? ''}`.trim().replace(/^,\s*/,''), note:newOrder.note, price:parseFloat(newOrder.price)||0, createdAt:nowISO(), updatedAt:nowISO() };
    saveOrder(order); setShowNew(false); loadOrders();
  }
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        ...
        <TouchableOpacity style={s.addBtn} onPress={openNew}><Text style={s.addBtnText}>+ Nová zakázka</Text></TouchableOpacity>
      </View>
      <View style={{ paddingHorizontal:S.base, paddingBottom:S.sm }}>
        <TextInput style={s.search} value={search} onChangeText={setSearch} placeholder="Hledat..." placeholderTextColor={C.textTertiary} />
      </View>
      ...
      <FlatList data={orders} keyExtractor={o => o.id} contentContainerStyle={{ padding:S.base, gap:S.sm }}
        ...
        ListEmptyComponent={<View style={{ alignItems:'center', paddingTop:60 }}><Text style={{ color:C.textTertiary, fontSize:F.lg }}>{filter==='dnes'?'Dnes žádné zakázky':'Žádné zakázky'}</Text></View>}
      />
          ...
          <View style={s.modal}>
            ...
            <ScrollView>
              ...
              <ScrollView style={s.customerList} nestedScrollEnabled>
                {customers.map(c => (
                  <TouchableOpacity key={c.id} style={[s.customerItem, newOrder.customerId===c.id && s.customerItemActive]} onPress={() => setNewOrder(p => ({...p, customerId:c.id}))}>
                    <Text style={[s.customerItemText, newOrder.customerId===c.id && {color:C.primary}]}>{c.firstName} {c.lastName} — {c.city}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            ...
            <View style={{ flexDirection:'row', gap:S.sm, marginTop:S.base }}>
              <TouchableOpacity style={s.cancelBtn} onPress={() => setShowNew(false)}><Text style={{ color:C.textSecondary }}>Zrušit</Text></TouchableOpacity>
              <TouchableOpacity style={s.createBtn} onPress={createOrder}><Text style={{ color:'#fff', fontWeight:'bold' }}>Vytvořit</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
