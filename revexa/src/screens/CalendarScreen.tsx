...
export default function CalendarScreen({ onSelectOrder }: Props) {
  ...
  return (
    <View style={{ flex:1, backgroundColor:C.bg }}>
      <View style={s.header}><Text style={s.title}>KALENDÁŘ</Text></View>
      <ScrollView>
        <View style={s.monthNav}>
          <TouchableOpacity onPress={() => month===0?[setMonth(11),setYear(y=>y-1)]:setMonth(m=>m-1)} style={s.arrow}><Text style={s.arrowText}>‹</Text></TouchableOpacity>
          <Text style={s.monthLabel}>{MONTH_NAMES[month]} {year}</Text>
          <TouchableOpacity onPress={() => month===11?[setMonth(0),setYear(y=>y+1)]:setMonth(m=>m+1)} style={s.arrow}><Text style={s.arrowText}>›</Text></TouchableOpacity>
        </View>
        <View style={s.dayNames}>{DAY_NAMES.map(d => <Text key={d} style={s.dayName}>{d}</Text>)}</View>
        <View style={s.grid}>
          ...
          {days.map(d => {
            ...
            return (
              <TouchableOpacity key={iso} style={[s.dayCell, isSel && s.dayCellSelected, isTod && !isSel && s.dayCellToday]} onPress={() => setSelectedDate(iso)}>
                <Text style={[s.dayNum, isSel && s.dayNumSelected]}>{d.getDate()}</Text>
                {(orderMap[iso]??0) > 0 && <View style={s.dot}><Text style={s.dotText}>{orderMap[iso]}</Text></View>}
              </TouchableOpacity>
            );
          })}
        </View>
        <View style={{ padding:S.base, gap:S.sm }}>
          <Text style={s.dayTitle}>{new Date(selectedDate+'T12:00:00').toLocaleDateString('cs-CZ',{weekday:'long',day:'numeric',month:'long'})}</Text>
          {dayOrders.length === 0
            ...
            : dayOrders.map(o => (
              <TouchableOpacity key={o.id} style={s.orderRow} onPress={() => onSelectOrder(o.id)}>
                ...
                <Text style={[s.orderRowStatus,{color:STATUS_COLORS[o.status]}]}>{STATUS_LABELS[o.status]}</Text>
              </TouchableOpacity>
            ))
          }
        </View>
...
const s = StyleSheet.create({
  ...
  dayCell:{ width:'14.28%' as any, aspectRatio:1, alignItems:'center', justifyContent:'center', borderRadius:R.md },
