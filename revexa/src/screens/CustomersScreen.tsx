...
interface Props { onSelectCustomer: (id: string) => void; onNewCustomer: () => void; }
  ...
  return (
    <View style={{ flex:1, backgroundColor:C.bg }}>
      <View style={s.header}>
        ...
        <TouchableOpacity style={s.addBtn} onPress={onNewCustomer}><Text style={s.addBtnText}>+ Nový zákazník</Text></TouchableOpacity>
      </View>
      <View style={{ padding:S.base, paddingBottom:S.sm }}>
        <TextInput style={s.search} value={search} onChangeText={setSearch} placeholder="Hledat jméno, město, telefon..." placeholderTextColor={C.textTertiary} />
      </View>
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={s.card} onPress={() => onSelectCustomer(item.id)}>
            ...
            <View style={{ flex:1 }}>
              ...
              <Text style={s.addr}>{item.street}, {item.city}</Text>
...
const s = StyleSheet.create({
  ...
  addr:{ color:C.textSecondary, fontSize:F.sm },
