...
export function initDb() {
  const db = getDb();
  db.execSync(`
    CREATE TABLE IF NOT EXISTS customers (id TEXT PRIMARY KEY, firstName TEXT NOT NULL, lastName TEXT NOT NULL, phone TEXT, email TEXT, street TEXT, city TEXT, zip TEXT, note TEXT, createdAt TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS chimneys (id TEXT PRIMARY KEY, customerId TEXT NOT NULL, label TEXT NOT NULL, type TEXT, fuel TEXT, note TEXT);
    CREATE TABLE IF NOT EXISTS orders (id TEXT PRIMARY KEY, customerId TEXT NOT NULL, chimneyId TEXT, orderNumber TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'nova', scheduledDate TEXT, completedAt TEXT, address TEXT, note TEXT, price REAL, createdAt TEXT NOT NULL, updatedAt TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS inspections (id TEXT PRIMARY KEY, orderId TEXT NOT NULL, chimneyId TEXT, reportNumber TEXT, inspectionDate TEXT NOT NULL, checkItems TEXT NOT NULL, overallResult TEXT NOT NULL, coMeasurement REAL, notes TEXT, recommendations TEXT, signatureBase64 TEXT, createdAt TEXT NOT NULL);
  `);
  const count = db.getFirstSync<{count:number}>('SELECT COUNT(*) as count FROM customers');
  if (count && count.count === 0) {
    const now = new Date().toISOString();
    const today = new Date().toISOString().split('T')[0];
    db.execSync(`
      INSERT INTO customers VALUES ('c1','Jan','Novák','603123456','jan@email.cz','Hlavní 12','Praha','11000','','${now}');
      INSERT INTO customers VALUES ('c2','Marie','Svobodová','607654321','','Nová 5','Brno','60200','','${now}');
      INSERT INTO chimneys VALUES ('ch1','c1','Komín č.1','průduch','dřevo','');
      INSERT INTO orders VALUES ('o1','c1','ch1','#0001','nova','${today}',NULL,'Hlavní 12, Praha','Pravidelná kontrola',1500,'${now}','${now}');
      INSERT INTO orders VALUES ('o2','c2',NULL,'#0002','probihajici','${today}',NULL,'Nová 5, Brno','Čištění',1200,'${now}','${now}');
    `);
  }
}
...
export function getStats() {
  const db = getDb();
  const total = db.getFirstSync<{count:number}>('SELECT COUNT(*) as count FROM orders')?.count??0;
  const done = db.getFirstSync<{count:number}>("SELECT COUNT(*) as count FROM orders WHERE status='dokoncena'")?.count??0;
  const revenue = db.getFirstSync<{sum:number}>("SELECT COALESCE(SUM(price),0) as sum FROM orders WHERE status='dokoncena'")?.sum??0;
  const thisMonth = new Date().toISOString().slice(0,7);
  const monthDone = db.getFirstSync<{count:number}>(`SELECT COUNT(*) as count FROM orders WHERE status='dokoncena' AND scheduledDate LIKE '${thisMonth}%'`)?.count??0;
