...
interface NavItem {
  id: AppSection | 'dashboard' | 'catalog' | 'settings';
  label: string;
  icon: string;
  disabled?: boolean;
}
    ...
    <View style={s.sidebar}>
      ...
      <View style={s.logoArea}>
        <View style={s.logoIcon}>
          <Text style={s.logoIconText}>R</Text>
        </View>
      ...
      <View style={s.navList}>
        {NAV_ITEMS.map(item => {
          ...
          return (
            <TouchableOpacity
              key={item.id}
              style={[s.navItem, isActive && s.navItemActive, item.disabled && s.navItemDisabled]}
              onPress={() => !item.disabled && onSelect(item.id as AppSection)}
              activeOpacity={item.disabled ? 1 : 0.75}
            >
              <Text style={s.navIcon}>{item.icon}</Text>
              <Text style={[s.navLabel, isActive && s.navLabelActive, item.disabled && s.navLabelDim]}>
                {item.label}
              </Text>
...
const s = StyleSheet.create({
  ...
  syncRow: { flexDirection: 'row', alignItems: 'center', gap: S.xs },
